# 🍕 PizzaHub – Разворачивание проекта с Docker
...
# 2️⃣ Создаем файл окружения и настраиваем его
cp .env.example .env
...
# 5️⃣ Генерируем ключ приложения
sudo docker exec -it app php artisan key:generate

# 6️⃣ Выполняем миграции
sudo docker exec -it app php artisan migrate

# 7️⃣ Заполняем базу тестовыми данными
sudo docker exec -it app php artisan db:seed

# 📂 После выполнения db:seed в папке postman будут сгенерированы файлы:
